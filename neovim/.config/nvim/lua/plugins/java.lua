...
return {
  ...
  -- Ensure java debugger and test packages are installed.
  {
    ...
    opts = function()
      ...
      -- Attach to cubchicken debug host/port (currently hard-coded to 3100 in ctl.sh)
      local dap = require('dap')
      dap.configurations.java = {
        {
          type = 'java',
...

  {
    ...
    opts = function(_, opts)
      ...
      --vim.api.nvim_create_autocmd({ "BufWritePost" }, { pattern = { "*.java" }, callback = function() local _, _ = pcall(vim.lsp.codelens.refresh) end,
          ...
          jdtls.handlers = {
            ...
            ['$/progress'] = function(err, result, ctx)
              ...
              if not err then
                ...
                if result.value.kind == 'end' and not tracking[result.token] then
                  return
                end
              end
              if result and result.token then
                if result.value.kind == 'end' then
                  tracking[result.token] = nil
                else
                  tracking[result.token] = true
                end
              end

              return progress_handler(err, result, ctx)
            end,
          }
        end,
